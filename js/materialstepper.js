  ...
  (function () {
    ...
    function MStepper(elem) {
      ...
      _defineProperty(this, "_methodsBindingManager", function (steps) {
        ...
        var bindOrUnbind = unbind
          ? removeMultipleEventListeners
          : addMultipleEventListeners; // Sets the binding function
        ...
        var bindEvents = function bindEvents(step) {
          ...
          if (options.stepTitleNavigation)
          ...
          if (inputs.length)
            bindOrUnbind(inputs[inputs.length - 1], "keydown", tabbingDisabler); // Binds to the submit button an internal handler to manage validation

          if (submitButtons && form && options.validationFunction)
            bindOrUnbind(submitButtons, "keydown", _formSubmitHandler);
          return step;
        }; // Calls the binder function in the right way (if it's a unique step or multiple ones)

        if (steps instanceof Element) bindEvents(steps);
        else
          nodesIterator(steps, function (step) {
      ...
      _defineProperty(this, "_openAction", function (step, cb) {
        ...
        if (
          ...
          !stepper.classList.contains(classes.HORIZONTALSTEPPER)
        ) {
          ...
          if (!skipAutoFocus) {
            _slideDown(stepContent, classes.ACTIVESTEP, step, function () {
              ...
              var nextStepInputs = stepContent.querySelector(
                "input, select, textarea"
              ); // Focus on the first input of the next step (temporarily disabled)

              if (options.autoFocusInput && nextStepInputs)
                nextStepInputs.focus();
        ...
        if (
          ...
          if (cb) {
            ...
            var waitForTransitionToCb = function waitForTransitionToCb(e) {
              ...
              _smartListenerUnbind(
                stepContent,
                "transitionend",
                waitForTransitionToCb
              ); // Calls the callback
      ...
      _defineProperty(this, "_stepTitleClickHandler", function (e) {
        ...
        if (stepper.classList.contains(classes.LINEAR)) {
          ...
          var clickedStepIndex = Array.prototype.indexOf.call(
            steps,
            clickedStep
          ); // If the step clicked is the next one, calls nextStep(), if it's the previous one, calls prevStep(), otherwise do nothing

          if (clickedStepIndex == active.index + 1) nextStep();
          else if (clickedStepIndex == active.index - 1) prevStep();
        } else {
      ...
      _defineProperty(this, "nextStep", function (cb, skipFeedback, e) {
        ...
        if (feedbackFunction && !skipFeedback) {
          ...
          if (showFeedbackPreloader && !active.step.dataset.nopreloader)
            activateFeedback(); // Calls the feedbackFunction

          window[feedbackFunction](
            destroyFeedback,
            form,
            active.step.querySelector(".".concat(classes.STEPCONTENT))
      ...
      _defineProperty(this, "wrongStep", function () {
        ...
        var inputs = getSteps().active.step.querySelectorAll(
          "input, select, textarea"
        ...
        var removeWrongOnInput = function removeWrongOnInput() {
          ...
          MStepper.removeMultipleEventListeners(
            inputs,
            "input",
            removeWrongOnInput
          );
      ...
      _defineProperty(this, "activateFeedback", function () {
        ...
        content.insertAdjacentHTML(
          "afterBegin",
          '<div class="'
            .concat(classes.PRELOADERWRAPPER, '">')
            .concat(options.feedbackPreloader, "</div>")
      ...
      _defineProperty(this, "destroyFeedback", function (triggerNextStep) {
        ...
        if (
          !activeStep ||
          !activeStep.classList.contains(classes.FEEDBACKINGSTEP)
        )
          return; // Removes the FEEDBACKINGSTEP class from the step

        activeStep.classList.remove(classes.FEEDBACKINGSTEP); // Gets the preloader div

        var fbDiv = activeStep.getElementsByClassName(
          classes.PRELOADERWRAPPER
        )[0]; // Removes the preloader div
      ...
      _defineProperty(this, "getSteps", function () {
        ...
        var activeStep = stepper.querySelector(
          "li.".concat(classes.STEP, ".").concat(classes.ACTIVESTEP)
        );
      ...
      _defineProperty(this, "activateStep", function (elements, index) {
        ...
        var referenceStep = before
          ? currentSteps[index]
          : currentSteps[currentSteps.length - 1]; // Stores a let variable to return the right element after the activation
        ...
        if (typeof elements === "string") {
          ...
          referenceStep.insertAdjacentHTML(
            before ? "beforeBegin" : "afterEnd",
            elements.trim()
          ); // Defines the inserted element as the returnableElement

          returnableElement = before
            ? referenceStep.previousSibling
            : referenceStep.nextSibling; // Activates (slideDown) the step
          ...
          elements.forEach(function (element) {
            ...
            var addedStep = before
              ? referenceStep.previousSibling
              : referenceStep.nextSibling; // Adds each element to the returnableElement array
          ...
          var rigthFunction = before
            ? stepper.insertBefore
            : stepper.appendChild; // Insert it/them with the rigthFunction and sets the returnableElement
          ...
          if (elements instanceof Element) _slideDown(returnableElement);
          else
            nodesIterator(returnableElement, function (appendedElement) {
      ...
      _defineProperty(this, "deactivateStep", function (elements) {
        ...
        if (elements instanceof Element) doIt(elements);
        else if (
          elements instanceof HTMLCollection ||
          elements instanceof NodeList
        )
          nodesIterator(elements, function (element) {
            return doIt(element);
      ...
      _defineProperty(this, "_slideDown", function (element, className) {
        ...
        var endSlideDown = function endSlideDown(e) {
          ...
          MStepper.removeMultipleProperties(
            element,
      ...
      _defineProperty(this, "_validationFunctionCaller", function () {
        var options = _this.options,
          ...
          form = _this.form,
          classes = _this.classes;
        return options.validationFunction(
          form,
          getSteps().active.step.querySelector(".".concat(classes.STEPCONTENT))
        );
      });
      ...
      _defineProperty(this, "_smartListenerBind", function (el, event, fn) {
        ...
        if (similar) {
          ...
          for (var i = 0; i < listenerStore.length; i++) {
            ...
            if (listener.event === event && listener.el.isSameNode(el))
              listener.el.removeEventListener(listener.event, listener.fn); // Calls the binded function if requested
          ...
          if (existentOne !== -1) {
            ...
            existentOne.el.removeEventListener(
              existentOne.event,
              existentOne.fn
            );
    ...
    _createClass(MStepper, null, [
      {
        ...
        value: function addMultipleEventListeners(elements, event, fn) {
          ...
          if (elements instanceof Element)
            return elements.addEventListener(event, fn, passive);
        ...
        value: function removeMultipleEventListeners(elements, event, fn) {
          var passive =
            arguments.length > 3 && arguments[3] !== undefined
              ...
              : false;
          if (elements instanceof Element)
            return elements.removeEventListener(event, fn, passive);
      ...
      },
      {
        ...
        value: function defaultValidationFunction(
          stepperForm,
          activeStepContent
        ) {
          var inputs = activeStepContent.querySelectorAll(
